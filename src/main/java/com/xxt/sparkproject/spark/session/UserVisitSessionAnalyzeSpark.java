...
import com.xxt.sparkproject.conf.ConfigurationManager;
import com.xxt.sparkproject.constant.Constants;
import com.xxt.sparkproject.dao.*;
import com.xxt.sparkproject.dao.factory.DAOFactory;
import com.xxt.sparkproject.domain.*;
import com.xxt.sparkproject.test.MockData;
import com.xxt.sparkproject.util.*;
 ...
 * 字段中
 * <p>
 * 接着J2EE平台会执行的spark-submit shell脚本，并将taskid作为参数传递给spark-submit shell脚本
...
//		sc.checkpointFile("hdfs://");
        ...
        //		获取 sessionid2 到访问行为数据的映射的RDD
        JavaPairRDD<String, Row> sessionid2actionRDD = getSessionid2ActionRDD(actionRDD);

        /**
         * 持久化， 对RDD调用persist()方法，并传入一个持久化级别
         *
...
//		sessionid2actionRDD.checkpoint();
        ...
        /**
         * 特别说明
         * 知道，要将上一个功能的session聚合统计数据获取到，就必须是在一个action操作触发job之后
         ...
         * 		有shfufle的算子，很容易导致数据倾斜，一旦数据倾斜，简直就是性能杀手
         * 4、无论做什么功能，性能第一
         ...
         *
         * 		如果采用第一种实现方案，那么其实就是代码划分（解耦合、可维护）优先，设计优先
...
//		return actionDF.javaRDD().repartition(1000);
    ...
    /**
     * 获取 sessionid2 到访问行为数据的映射的RDD  -> (sessionId , row )
     *
     * @param actionRDD
     * @return
     */
        ...
        return actionRDD.mapPartitionsToPair(new PairFlatMapFunction<Iterator<Row>, String, Row>() {
            ...
            public Iterable<Tuple2<String, Row>> call(Iterator<Row> iterator)
                ...
                while (iterator.hasNext()) {
                    ...
                    System.out.println("row.getString(2)----------" + row.getString(2));
                }
        ...
        JavaPairRDD<Long, String> userid2PartAggrInfoRDD = sessionid2ActionsRDD.mapToPair(
                ...
                new PairFunction<Tuple2<String, Iterable<Row>>, Long, String>() {
                    ...
                    public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple)
                        ...
                        // 返回的数据格式，即使<sessionid,partAggrInfo>
                        // 但是，这一步聚合完了以后，其实，是还需要将每一行数据，跟对应的用户信息进行聚合
                        // 问题就来了，如果是跟用户信息进行聚合的话，那么key，就不应该是sessionid
                        // 就应该是userid，才能够跟<userid,Row>格式的用户信息进行聚合
                        // 如果这里直接返回<sessionid,partAggrInfo>，还得再做一次mapToPair算子
                        // 将RDD映射成<userid,partAggrInfo>的格式，那么就多此一举

                        // 所以，这里其实可以直接，返回的数据格式，就是<userid,partAggrInfo>
...
//				});
        ...
        JavaPairRDD<String, String> filteredSessionid2AggrInfoRDD = sessionid2AggrInfoRDD.filter(
                ...
                new Function<Tuple2<String, String>, Boolean>() {
                    ...
                    public Boolean call(Tuple2<String, String> tuple) throws Exception {
                        ...
                        // 按照搜索词进行过滤
                        // 的session可能搜索了 火锅,蛋糕,烧烤
                        // 的筛选条件可能是 火锅,串串香,iphone手机
    ...
    private static void randomExtractSession(
        ...
        /**
         * 思考一下：这里不要着急写大量的代码，做项目的时候，一定要用脑子多思考
         *
         ...
         * 这个时候，就会发生数据倾斜
         *
         * 就用这个countByKey操作，给大家演示第三种和第四种方案
         *
         */
        ...
        int extractNumberPerDay = 0;
        if (!dateHourCountMap.isEmpty()) {
            extractNumberPerDay = 100 / dateHourCountMap.size();
        }
         ...
         * 之前是直接在算子里面使用了这个map，那么根据刚才讲的这个原理，每个task都会拷贝一份map副本
         * 还是比较消耗内存和网络传输性能的
         *
         * 将map做成广播变量
         *
        ...
        // 如果发现某个session恰巧在指定的这天这小时的随机抽取索引上
...
//		});
    ...
    private static JavaPairRDD<Long, Long> getClickCategoryId2CountRDD(
         ...
         * 点击行为的数据其实只占总数据的一小部分
         * 所以过滤以后的RDD，每个partition的数据量，很有可能跟之前说的一样，会很不均匀
         * 而且数据量肯定会变少很多
         *
